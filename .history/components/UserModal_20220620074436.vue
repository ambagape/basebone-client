<template>
  <v-dialog max-width="500px" v-model="open">
    <v-card class="p-5">
      ...
      <v-form>
        <v-text-field  v-if="_id" disabled v-model="_id"></v-text-field>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.stop="saveUser" color="green">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    saveUser() {
      ...
      if(cardData._id){
        this.$userService.updateUser(cardData);
      }else{
        this.$userService.createUser(cardData);
      }
      this.open = false;
    }
  },
  created() {
    eventBus.$on("open-add-user-modal", (data) => {
      if (data) {
        this._id = data._id;
        ...
        this.free_premium_items = data.free_premium_items;
      }
      this.open = true
    });
  },
